...
public class BossEnemy extends Rectangle implements Enemy {
    private int blood_ = BLOOD[BOSS_ENEMY];
    private final int MAX_BLOOD_ = BLOOD[BOSS_ENEMY];
    private final int ARMOR_ = ARMOR[BOSS_ENEMY];
    private final double SPEED_ = SPEED_ENEMIES[BOSS_ENEMY];
    private final int REWARD_ = REWARD[BOSS_ENEMY];
    private double x_;
    private double y_;
    private double dx_;
    private double dy_;
    ...
    public BossEnemy() {
        setBounds( spawners.get(points_appear).x + (TILE_SIZE- ENEMIES_SIZE[BOSS_ENEMY])/2, spawners.get(points_appear).y + (TILE_SIZE- ENEMIES_SIZE[BOSS_ENEMY])/2, ENEMIES_SIZE[BOSS_ENEMY], ENEMIES_SIZE[BOSS_ENEMY]);
        x_ = x;
        y_ = y;
    ...
    public void draw(Graphics g) {
        ...
        g.drawImage(img_Enemies[BOSS_ENEMY], x, y, width, height, null);
