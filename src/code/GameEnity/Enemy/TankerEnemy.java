...
public class TankerEnemy extends Rectangle implements Enemy {
    private int blood_ = BLOOD[TANKER_ENEYMY];
    private final int MAX_BLOOD_ = blood_;
    private final int ARMOR_ = ARMOR[TANKER_ENEYMY];
    private final double SPEED_ = 0.1;
    private final int REWARD_ = REWARD[TANKER_ENEYMY];
    private double x_;
    private double y_;
    private double dx_;
    private double dy_;
    ...
    public TankerEnemy() {
        setBounds(spawners.get(points_appear).x + (TILE_SIZE - ENEMIES_SIZE[TANKER_ENEYMY]) / 2, spawners.get(points_appear).y + (TILE_SIZE - ENEMIES_SIZE[TANKER_ENEYMY]) / 2, ENEMIES_SIZE[TANKER_ENEYMY], ENEMIES_SIZE[TANKER_ENEYMY]);
        x_ = x;
        y_ = y;
