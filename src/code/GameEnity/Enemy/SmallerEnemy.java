...
public class SmallerEnemy extends Rectangle implements Enemy {
    ...
    private final int MAX_BLOOD_ = BLOOD[SMALLER_ENEMY];
    private final int ARMOR_ = ARMOR[SMALLER_ENEMY];
    private final double SPEED_ = SPEED_ENEMIES[SMALLER_ENEMY];
    ...
    public void draw(Graphics g) {
        ...
        g.drawImage(img_Enemies[SMALLER_ENEMY], x, y, width, height, null);
    ...
    public void determine_p_next() {
        ...
        if (i > 0 && map[i][j] > map[i-1][j] && map[i-1][j] != MOUNTAIN) {
            ...
            --j;
            dx_ = -SPEED_;
        } else if (j < FIELD_COLUMNS-1 && map[i][j] > map[i][+1] && map[i][j+1] != MOUNTAIN) {
            ++j;
            dx_ = SPEED_;
        }
